:PROPERTIES:
:LAST_MODIFIED: [2026-08-16 Sun 20:32]
:END:
           ...
           (let* ((image-url
                  ...
                  (description
                   (if (string-empty-p description)
                     ...
                     (concat "\n" description)))
                  (artist-node
                   (if (member "person" (cashpw/org-get-filetags))
                       (save-excursion
                         (goto-char (point-min))
                         (org-node-at-point))
                     (org-node-read)))
                  (artist-tag
                   (when artist-node
                     (-->
                      (org-mem-entry-title artist-node)
                      (downcase it)
                      (replace-regexp-in-string " " "_" it)
                      (replace-regexp-in-string "-" "_" it))))
                  (tags (org-make-tag-string (list "photo" artist-tag)))
                  (artist-link
                   (org-link-make-string (format "id:%s" (org-mem-entry-id artist-node))
                                         (org-mem-entry-title artist-node)))
                  (image-link
                   (progn
