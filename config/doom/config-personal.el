...
(defcustom cashpw/org-gcal--profile-sleep
  `(:fetch-file-alist (("amc7oe0cqlg989fda4akqjl2f8@group.calendar.google.com" . ,(s-lex-format "${cashpw/path--notes-dir}/calendar-sleep.org")))
    :client-id "878906466019-a9891dnr9agpleamia0p46smrbsjghvc.apps.googleusercontent.com"
    :client-secret ,(cashpw/get-secret "org-gcal--personal"))
  "Personal configuration for `org-gcal'."
  :group 'cashpw
  :type 'sexp)
...
(defun cashpw/org-gcal-fetch-sleep (n-days)
  ...
  (let ((calendar-path
        ...
        (previous-profile
         cashpw/org-gcal--current-profile))
    (cashpw/org-gcal-activate-profile
     cashpw/org-gcal--profile-sleep)
    (let ((org-agenda-files `(,calendar-path)))
      (org-gcal-fetch))
    (cashpw/org-gcal-activate-profile
     previous-profile))
  (remove-hook!
...
(defun cashpw/org-gcal-clear-and-fetch ()
  ...
  (let ((calendar-path (cdr (car org-gcal-fetch-file-alist))))
    (with-current-buffer (find-file-noselect calendar-path)
      ...
      ;; Insert a heading; `org-gcal' throws an error if we cancel the first event
      (goto-char (point-max))
      (insert "* Flashcards"))
    (let ((org-agenda-files `(,calendar-path)))
      (org-gcal-fetch))
