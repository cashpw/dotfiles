:PROPERTIES:
:LAST_MODIFIED: [2025-05-17 Sat 10:23]
:END:
...
(defun cashpw/browser-get-search-prefix-alist ()
  "Return alist of search prefixes."
  (-flatten
   (mapcar
    (lambda (search-engine)
      (mapcar
       (lambda (key)
         `(,key . ,(plist-get search-engine :search-prefix)))
       (plist-get search-engine :keys)))
    cashpw/browser-search-engines)))

(defun cashpw/browser-select-url ()
  "Select URL from history and search engines."
  (let* ((search-prefix-alist (cashpw/browser-get-search-prefix-alist))
         (history-alist
          (--map
           `(,it . ,it)
           (lru-cache-table cashpw/browser-history)))
         (selection (completing-read "URL/Query: "
                                     (append
                                      search-prefix-alist
                                      history-alist)
                                     nil
                                     'require-match)))
    (when (s-starts-with-p "@" selection))


    ))

(defun cashpw/browser-get-search-prefix ()
  "Return selected search prefix."
  (when (= (length cashpw/browser-search-engines) 0)
    (error "Must have at least one search engine"))
  (if (= 1 (length cashpw/browser-search-engines))
      (plist-get (nth 0 cashpw/browser-search-engines) :search-prefix)
    (let* ((search-prefixes (cashpw/browser-get-search-prefix-alist)))
      (cdr
       (assoc
        (completing-read "Search engine: " search-prefixes
                         nil
                         'require-match)
        search-prefixes)))))
...
(defun cashpw/eww ()
  "Open EWW."
  (interactive)
  (let* ((eww-search-prefix (cashpw/browser-get-search-prefix))
         (url
          (read-string "Query: ")
          ))
    (eww)))
...
(defun cashpw/browser-history-update ()
  ...
  (let ((url (plist-get eww-data :url)))
    (unless (--any
      ...
      (lru-put url t cashpw/browser-history))))
