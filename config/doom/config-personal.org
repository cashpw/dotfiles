...
<<<<<<< HEAD
#+RESULTS:
| /usr/local/google/home/cashweaver/proj/notes/bibliography.bib |
...
(setq
 org-fc-max-new-positions-per-day 1
 org-fc-bury-siblings nil
 org-fc--new-positions-remaining
 one-day-in-seconds 60 * 60 * 24
 org-fc-review-interval-seconds one-day-in-seconds
 org-fc-last-review nil)

(defun org-fc--get-next-review-time ()
  )
...
(defun org-fc-position--is-new (position)
  (= 0
     (org-fc-position-interval position)))
...
(org-fc-index-filter-due
 (org-fc-index org-fc-context-all))
#+end_src
...
#+RESULTS:
: cashweaver/anki-to-fc--get-positions
...
(defun cashweaver/anki-to-fc--cloze-with-source (heading-text data anki-note-id)
  (let* ((review-data (mapcar
         ...
         (fc-cloze-max (number-to-string
                        (length review-data)))
         (fc-cloze (cashweaver/anki-to-fc--convert-text heading-text)))
    (org-set-property "FC_TYPE" "cloze")
    (org-set-property "FC_CLOZE_MAX" fc-cloze-max)
    (org-set-property "FC_CLOZE_TYPE" "deletion")
    (insert (concat " " fc-cloze))
    (org-fc-review-data-set review-data)))

(defun cashweaver/anki-to-fc--default (heading-text data anki-note-id)
  (let* ((review-data (mapcar
                       (lambda (datum)
                         (list (plist-get datum :position)
                               (plist-get datum :ease)
                               ...
                               (plist-get datum :due)))
                       data)))
    (org-set-property "FC_TYPE" "normal")
