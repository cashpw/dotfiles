:PROPERTIES:
:LAST_MODIFIED: [2026-04-10 Fri 14:31]
...
#+begin_src emacs-lisp :tangle config-personal.el :results none
(defun number-to-words-clisp (n callback)
  "Invoke clisp to get the written representation of N and call CALLBACK with the result."
  (cashpw/async-shell-command-to-string
   (format "clisp -q -norc -x '(format t \"~R\" %d)'" n)
   callback))
...
#+begin_src emacs-lisp :tangle config-personal.el :results none
(defun cashpw/url-get-title (url callback)
  "Fetch the title of URL and call CALLBACK with the result."
  (cashpw/async-shell-command-to-string
   (concat
...
(defun cashpw/ytt-api-transcript (youtube-video-id callback)
  "Fetch transcript of YOUTUBE-VIDEO-ID and call CALLBACK with the result."
  (cashpw/async-shell-command-to-string
   (format "youtube_transcript_api --format text %s" youtube-video-id)
   callback))

(defun llm-prompts-prompt-extract-wisdom-yt (youtube-url callback)
  "Fetch transcript for YOUTUBE-URL and call CALLBACK with the wisdom extraction prompt."
  (cashpw/ytt-api-transcript
   (cashpw/youtube--video-id-from-url youtube-url)
   (lambda (transcript)
     (funcall callback
              (format "%s\n\n%s" transcript llm-prompts-prompt--summarize)))))
#+end_src
