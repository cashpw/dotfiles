:PROPERTIES:
:LAST_MODIFIED: [2025-07-09 Wed 09:50]
:END:
...
(request
  ...
  :params `(("appid" . ,cashpw/openweather-api-key)
            ("units" . "imperial")
...
#+begin_src emacs-lisp
(let* ((data cashpw/openweather-data)
       (data-points (alist-get 'list data)))
  (dotimes (i (length data-points))
    (let* ((data-point (elt data-points i))
           (main (alist-get 'main data-point)))
      (message "%s: high:%s, low:%s"
               (format-time-string "%F %T" (seconds-to-time (alist-get 'dt data-point)))
               (alist-get 'temp_max main)
               (alist-get 'temp_min main)
               ))))
#+end_src
