...
- **Markdown Link Formatting**: When using backticks in links, you MUST place the backticks inside the square brackets (e.g., `[`file.py`](file://...)`). Never place backticks outside the square brackets (e.g., do NOT write `` `[file.py](file://...)` ``), as this breaks link rendering.
- **Spacing Constraints**: 
